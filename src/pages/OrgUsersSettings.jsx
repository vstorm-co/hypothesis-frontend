...
import { getOrganizationData, removeUserFromOrganization, revokeUsersAdmins, setUsersAdmins } from "../store/organizations-slice";
...
export function OrgUsersSettings(props) {
  ...
  const userToRemove = useSignal(0);
  ...
  function handleRemoveUserFromOrganization() {
    let data = {
      user_ids: [`${userToRemove.value}`],
      admins_ids: [`${userToRemove.value}`],
    }

    dispatch(removeUserFromOrganization({ uuid: organization.uuid, data, }))
  ...
  return (
    <div className={'relative w-full'}>
                ...
                <tbody key={selectedUsers.value.length}>
                  {organization.users.map((user) =>
                    <tr key={user.id} className={'border-b border-[#DBDBDB] ' + (selectedUsers.value.some(u => u.id === user.id) ? 'bg-[#FAFAFA]' : '')}>
                      ...
                      <td>
                        <div onClick={() => { handleAskRemove(user.id) }} className={'flex items-center text-[#747474] group-hover:text-[#DBDBDB]'}>
      ...
      <div className={'fixed rounded left-1/2 top-10 flex flex-col bg-[#020202] text-white w-[350px] ' + (confirmRemove.value ? '' : 'hidden')}>
        <div className='px-4 py-2'>
          Are you sure you want to delete this User?
        </div>
        <div className='flex justify-center items-center gap-4 border-t border-[#747474] py-2'>
          <div onClick={() => { confirmRemove.value = false }} className={'px-2 py-1 cursor-pointer hover:bg-[#747474] rounded'}>Cancel</div>
          <div onClick={() => { handleRemoveUserFromOrganization() }} className={'bg-[#EF4444] px-2 py-1 rounded cursor-pointer'}>Delete</div>
        </div>
      </div>
    </div >
  )
