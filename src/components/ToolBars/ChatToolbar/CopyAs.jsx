...
class CopyAs extends Component {
  ...
  copyFormatted = () => {
    ...
    localStorage.setItem('ANT_defaultSaveAs', 'rtf');
    this.setState({ defaultSaveAs: 'rtf' })
    ...
    return (
      <div ref={this.CopyAsRef} className={'relative'}>
        <div className={"border border-[#DBDBDB] rounded cursor-pointer flex items-center"}>
          <div title={`Copy ${this.state.options.find(o => o.id === this.state.defaultSaveAs).label}`} onClick={this.handleCopyDefault} className={"p-1 text-sm hover:bg-[#F2F2F2] " + (this.props.showCopyAs.value ? 'bg-[#F2F2F2]' : '')}>
          ...
          <div onClick={this.toggleShowCopyAs} className={'border-l hover:bg-[#F2F2F2]'}>
            <div className={'px-2 py-2 flex gap-2 text-[#747474]'}>
              <span className={'uppercase font-bold text-xs'}>{this.state.defaultSaveAs}</span>
        ...
        <div className={"absolute z-50 border rounded w-[160px] right-0 top-10 bg-white " + (this.props.showCopyAs ? '' : 'hidden')}>
          <div className="">
            ...
            {this.state.options.map(option => (
              <div style={option.id === this.state.defaultSaveAs ? { boxShadow: '4px 0px 0px 0px #DBDBDB inset' } : {}} onClick={() => this.handleCopy(option.id)} className="text-sm flex justify-between items-center border-b py-2 px-[12px] text-[#202020] hover:bg-[#FAFAFA] cursor-pointer">
                {option.label}
